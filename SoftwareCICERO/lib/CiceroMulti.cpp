...
// Wrapper around the 16bit instruction for easy retrieval of type, data and easy printing.
    ...
    //TODO
    unsigned short Instruction::getChar1(){return this->getData()>>8;};
    unsigned short Instruction::getChar2(){return instr % ( 1 << (BITS_INSTR - BITS_INSTR_TYPE));};
	...
	void Instruction::print(int pc){
		...
		{
		case 0:
			printf("ACCEPT char1: %c, char2: %c\n", this->getData() >> 8, this->getChar2());
			break;
		case 1:
			printf("SPLIT\t {%d,%d} \n", pc+1, this->getData());
			break;
...
// Container for all the buffers - permits to instantiate a variable number of buffers.
	Buffers::Buffers(int n){ 
		...
		buffers.reserve(n);
		for (int i = 0; i < n; i++){
			buffers.push_back(queue<unsigned int>());
		}
	}
		...
		switch(stage12->getType()){
            ...
            case RANGE:
                ...
                if (char(stage12->getChar1())<=currentChar && (char(stage12->getChar2())>=currentChar)) {
                    valid = true;
                    newPC = CoreOUT(sCO12.getPC()+1, sCO12.getCC_ID()+1);
                    if (verbose) { printf("\t\tCharacters in range: input %c from %c to %c\n", currentChar, char(stage12->getChar1()), char(stage12->getChar2()));}
                } else {
                    if (verbose) { printf("\t\tCharacters not in range: input %c from %c to %c\n", currentChar, char(stage12->getChar1()), char(stage12->getChar2()));}
                    newPC = CoreOUT();
                }
                break;
