...
#define BITS_INSTR_TYPE 4
#define BITS_INSTR 20
...
namespace CiceroMulti {
	...
	enum InstrType {
        ...
        RANGE                 = 8,
        NOT_RANGE             = 9,
	}; //8 instruzioni quindi il range deve necessariamente aggiungere un bit all'opcode TODO
	...
	class Instruction {
		...
		public: 
            ...
            //TODO
            unsigned short getChar1();
            unsigned short getChar2();
	...
	class Buffers {
		private:
			std::vector<std::queue<unsigned int>> buffers;
			int HEAD;
			int size; //2**W 
		public:
			Buffers(int n);
