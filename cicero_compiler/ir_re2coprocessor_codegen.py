...
def Match(self,  pc, list_ir_instructions: list,  list_instructions: list):
		...
		list_instructions[pc] = anInstr
def MatchRange(self,  pc, list_ir_instructions: list,  list_instructions: list):
	next_pc = list_ir_instructions.index(self.children[0])
	assert pc +1 == next_pc, str(pc+1)+ " !== "+ str(next_pc)

	anInstr = ir_re2coprocessor.MatchRange(pc,self.char1,self.char2)
	list_instructions[pc] = anInstr

def NotMatchRange(self,  pc, list_ir_instructions: list,  list_instructions: list):
	next_pc = list_ir_instructions.index(self.children[0])
	assert pc +1 == next_pc, str(pc+1)+ " !== "+ str(next_pc)

	anInstr = ir_re2coprocessor.NotMatchRange(pc,self.char1,self.char2)
	list_instructions[pc] = anInstr
