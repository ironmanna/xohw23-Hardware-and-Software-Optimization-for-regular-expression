...
class MatchRange(IrInstr):
	def __init__(self, achar1, achar2):
		super().__init__()
		if isinstance(achar1, str):
			self.char1 = bytes(achar1, 'utf-8')[0]
		else:
			self.char1 = achar1
		if isinstance(achar2, str):
			self.char2 = bytes(achar2, 'utf-8')[0]
		else:
			self.char2 = achar2

	def dotty_repr(self):
		# printable Ascii \x20-\x7F
		char1 = chr(self.char1)
		if self.char1 < 32 or self.char1 > 127  or char1 == "\"\\":
			char1 = hex(self.char1)
		char2 = chr(self.char2)
		if self.char2 < 32 or self.char2 > 127  or char2 == "\"\\":
			char2 = hex(self.char2)
		return f"{id(self)} [label =\"{char1}\" color=\"black\" fillcolor=\"#ffa822\" style=\"filled\"] [label =\"{char2}\" color=\"black\" fillcolor=\"#ffa822\" style=\"filled\"]\n"

	def equiv(self, other):
		return super().equiv(other) and self.char1 == other.char1 and self.char2 == other.char2

class NotMatchRange(IrInstr):
	def __init__(self, achar1, achar2):
		super().__init__()
		if isinstance(achar1, str):
			self.char1 = bytes(achar1, 'utf-8')[0]
